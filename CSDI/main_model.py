...
class CSDI_base(nn.Module):
    def __init__(self, target_dim, config, device=device):
        ...
        ) ** 2 if config['diffusion']["schedule"] == "quad" else np.linspace(
        ...
        ) ** 2 if config_diff["schedule"] == "quad" else np.linspace(
    ...
    def get_randmask(self, observed_mask):
        rand_for_mask = torch.rand_like(observed_mask, dtype=torch.float32) * observed_mask
    ...
    def get_side_info(self, observed_tp, cond_mask):
        ...
        time_embed = self.time_embedding(observed_tp, self.emb_time_dim).unsqueeze(2).expand(-1, -1, K, -1)
        
        feature_embed = self.embed_layer(torch.arange(self.target_dim, device=self.device)).unsqueeze(0).unsqueeze(0).expand(B, L, -1, -1)
    ...
    def calc_loss(self, observed_data, cond_mask, observed_mask, side_info, is_train, set_t=-1):
        ...
        t = torch.ones(B, dtype=torch.long, device=self.device) * set_t if is_train != 1 else torch.randint(0, self.num_steps, [B], device=self.device)
        current_alpha = self.alpha_torch[t]
        noise = torch.randn_like(observed_data, dtype=torch.float32)
    ...
    def impute(self, observed_data, cond_mask, side_info, n_samples):
        B, K, L = observed_data.shape
        imputed_samples = torch.zeros(B, n_samples, K, L, device=self.device, dtype=torch.float32)

        for i in range(n_samples):
            current_sample = torch.randn_like(observed_data, dtype=torch.float32)
            for t in range(self.num_steps - 1, -1, -1):
                diff_input = self.set_input_to_diffmodel(current_sample, observed_data, cond_mask)
                predicted = self.diffmodel(diff_input, side_info, torch.tensor([t], device=self.device, dtype=torch.long))
                ...
                if t > 0:
                    ...
                    current_sample += sigma * torch.randn_like(current_sample, dtype=torch.float32)
    ...
    def forward(self, batch, is_train=1):
        ...
        cond_mask = gt_mask if is_train == 0 else self.get_hist_mask(observed_mask, for_pattern_mask=for_pattern_mask) if self.target_strategy != "random" else self.get_randmask(observed_mask)
...
class CSDI_Forecasting(CSDI_base):
    ...
    def get_side_info(self, observed_tp, cond_mask, feature_id=None):
        B, K, L = cond_mask.shape
        print("cond_mask shape", cond_mask.shape)
        ...
        print(f"time_embed after unsqueeze and expand shape: {time_embed.shape}")
