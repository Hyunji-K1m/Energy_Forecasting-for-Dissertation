...
from score_sde.score_fn import EpsilonTheta
from score_sde.sde_lib import VPSDE, subVPSDE, VESDE
from score_sde.losses import get_loss_fn
from score_sde.distribution_output import score_fn_output
# from .util import get_score_fn
from score_sde.sampling import get_sampling_fn
...
class ScoreGradTrainingNetwork(nn.Module):
    ...
    def __init__(
        ...
        input_size: int=320, #324
        num_layers: int=2,
        num_cells: int=2,
        cell_type: str='LSTM',
        history_length: int=767,
        context_length: int=365,
        prediction_length: int=14,#30
        lags_seq: List[int]=[1,7,14,30],
        **kwargs,
    ) -> None:
    ...
    def unroll(
        ...
        print(f"Calculated input size: {input_lags.shape[-1] + repeated_index_embeddings.shape[-1] + time_feat.shape[-1]}")
        # unroll encoder
